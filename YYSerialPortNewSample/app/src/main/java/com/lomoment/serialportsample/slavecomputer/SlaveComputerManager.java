package com.lomoment.serialportsample.slavecomputer;

import android.content.Context;
import android.util.Log;

import com.lomoment.serialportsample.entities.DeliverInfo;
 ...
 * @author libin
 * @date 2018/5/22
 * @Description Slave Computer
 */

public class SlaveComputerManager implements VendingMachineMananger.VendingmachineResponseListener, VendingMachineMananger.MachineQueryResponseListener {
    public static final String TAG = SlaveComputerManager.class.getSimpleName();
    //serial port pa
    public static final String SERICAL_PORT_PATH = "/dev/ttyS4";

    public static final int BAUDRATE = 115200;

    private static SlaveComputerManager mInstance;
    ...
    private SlaveComputerManager() {
        sendReq = new HashMap<>();
    }

    public static SlaveComputerManager getInstance() {
        if (mInstance == null) {
            synchronized (SlaveComputerManager.class) {
                if (mInstance == null) {
                    mInstance = new SlaveComputerManager();
                }
            }
        }
        return mInstance;
